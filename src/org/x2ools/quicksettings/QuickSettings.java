...
public class QuickSettings {
	...
	private static BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(
					X2oolsActivity.ACTION_WECHAT_SCAN_CHANGED)) {
				canAddScanTile = intent.getBooleanExtra(
						X2oolsActivity.KEY_WECHAT_SCAN, false);
				updateTileLayout();
			}
		}
	};
	...
	public static void handleLoadPackage(final LoadPackageParam lpparam)
		...
		Class<?> quickSettingsClass = XposedHelpers.findClass(
				CLASS_QUICK_SETTINGS, lpparam.classLoader);
		Class<?> phoneStatusBarClass = XposedHelpers.findClass(
				CLASS_PHONE_STATUSBAR, lpparam.classLoader);
		Class<?> panelBarClass = XposedHelpers.findClass(CLASS_PANEL_BAR,
				lpparam.classLoader);

		XposedBridge.hookAllConstructors(quickSettingsClass,
				quickSettingsConstructHook);
		...
		XposedHelpers.findAndHookMethod(quickSettingsClass, "addSystemTiles",
				ViewGroup.class, LayoutInflater.class,
	...
	private static XC_MethodHook quickSettingsConstructHook = new XC_MethodHook() {
		...
		protected void afterHookedMethod(final MethodHookParam param)
			...
			mContext = (Context) XposedHelpers.getObjectField(param.thisObject,
					"mContext");
			mX2oolContext = mContext.createPackageContext(
					X2oolsActivity.X2OOL_PACKAGE_NAME,
					Context.CONTEXT_IGNORE_SECURITY);
			mContainerView = (ViewGroup) XposedHelpers.getObjectField(
					param.thisObject, "mContainerView");
	...
	private static XC_MethodHook quickSettingsAddSystemTilesHook = new XC_MethodHook() {
		...
		protected void afterHookedMethod(final MethodHookParam param)
			...
			scanTile = new WechatScanTile(mContext, mX2oolContext, mStatusBar,
					mPanelBar);
	...
	public static class TileLayout {
		...
		public TileLayout(Context context, int numCols, int orientation,
				LabelStyle lStyle) {
			numColumns = numCols;
			labelStyle = lStyle;
			...
			try {
				...
				imageMarginBottom = res.getDimensionPixelSize(res
						.getIdentifier("qs_tile_margin_below_icon", "dimen",
								PACKAGE_NAME));
				imageSize = res.getDimensionPixelSize(res.getIdentifier(
						"qs_tile_icon_size", "dimen", PACKAGE_NAME));
			} catch (Resources.NotFoundException rnfe) {
				final Resources x2oolRes = mX2oolContext.getResources();
				imageMarginTop = x2oolRes
						.getDimensionPixelSize(R.dimen.qs_tile_margin_above_icon);
			...
			if (labelStyle == LabelStyle.HIDDEN) {
				imageMarginTop += TypedValue.applyDimension(
